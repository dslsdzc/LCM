...
- **Active channel** (Active Channel (pretrained LLM)): retrieves semantic-syntactic primitives from cognitive state to generate fluent, rich expression
...
LCM consists of four major modules connected end-to-end, forming a cognitive loop: **Perceptual Frontend, Multi-Lattice Memory, Zero-Parameter Inference Engine, Active Channel (pretrained LLM) (Active Channel)**. The Active Channel (pretrained LLM) shares token embedding and output projection `W_out` with the Cognitive LCM, with independent codebooks storing different types of knowledge.
...
graph TD
    ...
    subgraph Active Channel: Active Channel (pretrained LLM)
...
### 5.1 Stage 1: Active Channel (pretrained LLM) Training (Pure Language Model)

The Active Channel (pretrained LLM) trains as a standalone language model: `encoder → codebook retrieval-fusion → W_out`. Pure CE loss:

```
L_lang = cross_entropy(z_q @ W_out, targets)
```

Goal: codebook entries converge to stable semantic-syntactic primitives, enabling the Active Channel (pretrained LLM) to generate fluent text independently.

### 5.2 Stage 2: Dual LCM Joint Training

Load the trained Active Channel (pretrained LLM) as the active channel; the Cognitive LCM begins training:
...
- `L_active`: CE loss from active channel (Active Channel (pretrained LLM) retrieves primitives, fuses, outputs — rich expression).
