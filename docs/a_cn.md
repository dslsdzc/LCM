...
主动通道负责将认知状态 `z_q` 转换为流畅的自然语言。**尚未形成**——但形成过程已经启动。

**机制：Qwen 反向形成主动通道。** 认知训练时，Qwen 桥接的梯度（`L_active`）反向传播回认知系统，迫使 z_q 向对语言生成有用的方向收敛。这个反向过程本质上就是在训练主动通道——Qwen 的输出监督信号在"教会"认知系统如何从自身状态解码出语言。随着训练进行，认知系统逐渐学会绕过 Qwen、直接从 z_q 生成准确语义输出。

当前实现：z_q（256维）通过可训练的 `z_proj` 投影到 Qwen 的 896 维隐藏空间，Qwen 前 4 层 decoder 生成 logits，CE loss 的梯度经 Qwen（冻结）传回 z_q。
