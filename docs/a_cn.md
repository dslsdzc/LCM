...
LCM 由**感知前端、多格记忆体、零参数推理引擎、主动通道** 四大模块首尾相连，形成认知闭环。主动通道**尚未形成**，当前由 Qwen2.5-0.5B 临时桥接。此前尝试的 LangLCM（独立语言LCM）是过渡概念，不存在于最终架构。
...
主动通道是 Qwen 从认知状态 `z_q` 产生的**连续语义表示**返回给主模型，使认知系统的 latent space 获得语义组织能力。**尚未完全形成**——但形成过程已经启动。

**机制：Qwen 反向形成主动通道。** z_q 经 `z_proj` 投影到 Qwen 隐藏空间，Qwen 前 4 层 decoder 产生连续语义向量，其梯度反向传播回认知系统，迫使 z_q 向语义有序的方向收敛。这个反向过程本质上就是在训练主动通道——Qwen 提供的连续语义监督信号在"教会"认知系统如何组织有意义的语义空间。随着训练进行，认知系统逐渐学会产生语义结构清晰的 z_q，最终脱离 Qwen 独立维持语义连续性。

当前实现：z_q（256维）→ `z_proj` → Qwen 896d 隐藏空间 → Qwen 前 4 层 decoder → logits → CE loss → 梯度经 Qwen（冻结）传回 z_q。

此前尝试过 LangLCM（独立的 4 层 transformer decoder + 6 codebook soft read），是与认知LCM同构的完整LCM实例，codebook 存储语义-句法基元。但因容量不足、位置编码缺失等问题被放弃，**LangLCM 是过渡概念，不存在于最终架构**。代码保留在 `train/lang_lcm.py` 中供参考。

### 4.4 零参数推理引擎

LCM 的推理过程不依赖神经网络权重，而由零参数动态数据流推理机完成。**这里的"零参数"是字面意思：C 引擎代码中没有任何可训练或可更新的权重。codebook 和 W_out 作为外部数据由引擎加载读取，不是引擎的参数。** 推理引擎作为独立的认知计算机，接收多格记忆体的融合输出 `z_q` 并执行多步推理，其详细设计见《晶格认知模型推理引擎规范》（`c.md`）。

推理引擎输出 `z_q` 后，分两条路径：
- **被动通道**：`z_q @ W_out` 直接读出 token logits（诚实透明）
- **主动通道**：z_q 送入主动通道（当前为 Qwen 临时桥接，最终目标为模型自身生成）输出表达
...
训练分为**两个当前流程**和一个**历史流程**：

| 流程 | 模块 | 训练什么 | 关系 |
|------|------|---------|------|
| **认知训练** (`cog_train.py`) | encoder + 6 codebooks + W_out + 认知循环 | 全参数训练，包括码本 | 主训练流程，训练认知系统收敛 |
| **记忆训练** (`train_memory.py`) | 6 codebooks | 仅更新码本内容 | 辅助流程，可在推理部署后独立运行 |
| **(历史) LM预训练** (`train_lm.py`) | gen_head 解码器 | 仅解码器 | 已淘汰 |

认知训练和记忆训练**互补不互斥**：认知训练是主流程，同时训练认知系统和码本；记忆训练是专项持续学习，可在部署后独立更新码本而不影响认知系统。

### 5.1 Stage 1（历史）：冻结LLM 训练（纯语言模型）

> 此方案已淘汰，LangLCM 是过渡概念。当前主动通道使用 Qwen 临时桥接。代码保留在 `train/train_lang_lcm.py` 中。
...
V1（2026-06 至 2026-07）：[训练实验报告 V1（中文）](training_report_v1_cn.md)
