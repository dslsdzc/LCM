...
- **主动通道**（语言LCM）：从认知状态检索语义-句法基元，生成丰富流畅的表达
...
LCM 由**感知前端、多格记忆体、零参数推理引擎、语言LCM（主动通道）** 四大模块首尾相连，形成认知闭环。语言LCM 与认知LCM 共享 token embedding 和输出投影 `W_out`，codebook 各自独立存储不同知识。
...
语言LCM（LangLCM）取代了旧版本的轻量生成头（单层线性注意力+GLU），是**一个与认知LCM同构的完整LCM实例**。其 codebook 不存储认知概念，而是存储**语义-句法基元**——句式骨架、论元角色、常用搭配、语气风格等不同粒度的语言构造。
...
- **主动通道**：语言LCM 从 `z_q` 检索语义-句法基元，经融合 + W_out 输出丰富表达
...
### 5.1 Stage 1：语言LCM 训练（纯语言模型）

语言LCM 作为独立语言模型训练，结构为 `encoder → codebook检索融合 → W_out`，纯 CE loss：

```
L_lang = cross_entropy(z_q @ W_out, targets)
```

目标：codebook 条目收敛为稳定的语义-句法基元，使语言LCM 能独立生成流畅文本。

### 5.2 Stage 2：双LCM 联合训练

加载训练好的语言LCM 作为主动通道，认知LCM 开始训练：
...
- `L_active`：主动通道（语言LCM 检索融合后输出）CE loss（丰富表达）
